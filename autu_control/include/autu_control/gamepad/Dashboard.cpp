...
Dashboard::Dashboard(ros::NodeHandle *nh, QWidget *parent)
  ...
  robotSensors = nh->subscribe<sensor_data>(
      "pses_basis/sensor_data", 10, boost::bind(sensorCallback, _1, ui));
  carInfo = nh->subscribe<info_data>("pses_basis/car_info", 10,
                                     boost::bind(infoCallback, _1, ui));
  ...
  if (videoMode.compare("image_color") == 0) {
    cameraSub = nh->subscribe<image_msg>("kinect2/qhd/image_color", 10,
                                         boost::bind(cameraCallback, _1, ui));
    ...
    depthSub = nh->subscribe<image_msg>("kinect2/sd/image_depth", 10,
                                        boost::bind(depthCallback, _1, ui));
...
void sensorCallback(const sensor_data::ConstPtr &sensor, Ui::Dashboard *ui) {
  ...
  ui->sensor_hall_count->display(
      (int)sensor->hall_sensor_count); // <- this is an unsinged integer
  // battery sensor data
